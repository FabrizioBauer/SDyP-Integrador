...
short **vecindarioDeMoore(short *matrix[], short filas, short columnas, short posX, short posY)
{
...
void main(int argc, char *argv[])
{
    ...
    {
        #pragma omp parallel num_threads(threads) private(probContagio,random)
        {
            ...
            for (i = 0; i < filas; i++)
            {
                for (j = 0; j < columnas; j += 4)
                {
                    ...
                    case 4: //sano
                        ...
                        probContagio = (porcentajeConSintomas(vecindarioDeMoore(matrix, filas, columnas, j, i)) + susceptibilidad(matrix[i][j+1], matrix[i][j+2])) * 0.6 + 0.05;
