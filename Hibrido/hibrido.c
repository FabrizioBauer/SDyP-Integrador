...
short **vecindarioDeMoore(short *matrix[], short filas, short columnas, short posX, short posY, int id, int nproc)
{
...
void main(int argc, char *argv[])
{
    ...
    for(generacion = 0; generacion<semanas; generacion++)
    {
		...
		*/
        ...
        #pragma omp parallel num_threads(threads) private(random, probContagio)
        {
            ...
            for (i = 0; i < filas; i++)
            {
                for (j = 0; j < columnas; j += 4)
                {
                    ...
                    case 4: //sano
                        ...
                        probContagio = (porcentajeConSintomas(vecindarioDeMoore(matrix, filas, columnas, j, i, mytid, nproc)) + susceptibilidad(matrix[i][j+1], matrix[i][j+2])) * 0.6 + 0.05;
